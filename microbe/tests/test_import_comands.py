...
from microbe.external_apis.biosamples.api import API_ROOT as BSAPIROOT
...
MGAPIROOT = microbe_config.mgnify.api_root.rstrip("/")
...
def test_fetch_project_samples(requests_mock):
    requests_mock.get(
        f"{BSAPIROOT}/samples?filter=attr:project:MICROBE&size=200",
        json={
