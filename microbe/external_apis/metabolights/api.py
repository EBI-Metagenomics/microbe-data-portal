...
from microbe.external_apis.metabolights.auth import MTBLS_AUTH
from microbe.utils import microbe_config, clean_keys

API_ROOT = microbe_config.metabolights.api_root.rstrip("/")
...
def get_metabolights_assays(mtbls_accession: str, sample_accession: str) -> List[dict]:
    ...
    with closing(
        ...
        for row in reader:
            if (
                row.get(
                    microbe_config.metabolights.biosample_column_name_in_sample_table
                )
                == sample_accession
            ):
